...
- **GitHub releases must carry installable assets.** After `gh release create vX.Y.Z`, build the .aip (zip ONLY the app files: app.json, index.html, app.js, style.css, icon.svg, libs/ — EXCLUDE tests/, docs, .git) and upload it plus a .zip copy: `gh release upload vX.Y.Z term-coder-vX.Y.Z.aip term-coder-vX.Y.Z.zip`. A release without a .aip/.zip asset breaks ChatOSS's update flow ("This release has no .aip file to install").
