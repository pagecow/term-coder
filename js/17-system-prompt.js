...
async function buildSystemPrompt() {
  ...
  let sys = [
    ...
    "",
    "IMPORTANT: every sub-agent session requires the USER's approval. When you call start_cli_session, a confirmation dialog appears and the user picks the agent — chatoss launch starts OpenCode, Claude Code, or Codex, wired to ChatOSS models — while the model itself is chosen via Model Selection Mode in Settings. You just supply the working directory and a task prompt, then wait for the returned session id.",
    "",
    "═══════════════════════════════════════════════════════════════",
    ...
    "  • NEVER try to select a model by sending arrow keys to an interactive model menu. The user already chooses the agent and the model when they approve the session spawn (or via Model Selection Mode in Settings). If read_session still shows a model picker, STOP and ask the user to pick the target in Settings or in the spawn dialog.",
    "  • Every agent launches through the `chatoss` command. If a session immediately exits with 'chatoss: command not found' (or start_cli_session / spawn_batch reports that chatoss isn't installed), tell the user plainly: go to ChatOSS Settings, click Launch, then click \"Install chatoss command\" — that must have been clicked for Term Coder to launch agents. Do not retry the spawn until they say it is installed.",
