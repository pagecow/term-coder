...
// style collapsible container — a subtle, DIMMED disclosure row that is FULLY
// collapsed by default (just a "Thinking…" header, no preview), so the
// reasoning never competes with the actual response text. Clicking the header
// expands the full reasoning inline; clicking again collapses it. The thinking
// text is visually distinct from the response (muted/monospace). While
...
test("source: createThinkingWidget builds a collapsible header (.think-toggle)", () => {
  const m = SRC.match(/function createThinkingWidget[\s\S]*?\n\}/);
  assert(m, "createThinkingWidget not found in app.js");
  const body = m[0];
  assert(/think-toggle/.test(body), "widget must create a .think-toggle header");
  assert(/Thinking…/.test(body), "streaming header must read 'Thinking…'");
  assert(/Thought process/.test(body), "finalized header must read 'Thought process'");
});

test("source: CSS collapses the body fully (max-height:0), not a preview", () => {
  ...
  // The base .think-body rule (collapsed) must be max-height:0 so the reasoning
  // is fully hidden and never competes with the response text.
  ...
  assert(/max-height:\s*0\b/.test(bodyRule[0]),
    "collapsed .think-body should be max-height:0 (fully hidden, found: " + bodyRule[0].trim() + ")");
...
test("behavior: streaming widget stays collapsed while tokens accumulate", () => {
  ...
  // how much thinking streams in, the widget must remain collapsed — the word
  // count ticks up in the header, but the reasoning never auto-expands.
  ...
  w._update("first chunk of reasoning\n\nmore reasoning that grows longer and longer " +
    "until it overflows any preview window");
  assert.strictEqual(w.getAttribute("data-state"), "collapsed",
    "must stay collapsed even when text grows long (stays a dim summary)");
  // The accumulated text is present in the inner body.
  const inner = find(w, findByClass("think-inner"));
  assert(inner && /overflows any preview window/.test(inner.textContent),
...
test("behavior: the header toggle expands to open and back to collapsed", () => {
  const w = makeWidget("reasoning line one\nreasoning line two", { streaming: false });
  const header = find(w, findByClass("think-toggle"));
  assert(header, ".think-toggle header must exist");
  assert.strictEqual(header.getAttribute("aria-expanded"), "false");

  // Click header → opens.
  header._listeners.click();
  assert.strictEqual(w.getAttribute("data-state"), "open", "header click must expand the widget");
  assert.strictEqual(header.getAttribute("aria-expanded"), "true");

  // Click header again → collapses back.
  header._listeners.click();
  assert.strictEqual(w.getAttribute("data-state"), "collapsed", "second header click must collapse");
  assert.strictEqual(header.getAttribute("aria-expanded"), "false");
});
